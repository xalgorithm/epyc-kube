...
if kubectl exec -n monitoring $POD_NAME -- grep -q "root_url = https://grafana.xalg.im" /etc/grafana/grafana.ini; then
...
OAUTH_RESPONSE=$(curl -s -I https://grafana.xalg.im/login/generic_oauth)
if echo "$OAUTH_RESPONSE" | grep -q "login.xalg.im"; then
    ...
    if [ "$REDIRECT_URI" == "https://grafana.xalg.im/login/generic_oauth" ]; then
...
if curl -s http://localhost:8080/realms/master/.well-known/openid_configuration > /dev/null 2>&1; then
    ...
    ADMIN_TOKEN=$(curl -s -X POST \
        ...
        -d "username=xalg&password=Pr1amsf0lly!&grant_type=password&client_id=admin-cli" \
...
echo "  - Grafana URL: https://grafana.xalg.im"
...
echo "1. Visit: https://grafana.xalg.im/login"
