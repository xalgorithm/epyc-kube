...
# Verifies the integrity and contents of EthosEnv backups
...
echo -e "${BLUE}EthosEnv Backup Verification${NC}"
echo -e "${BLUE}============================${NC}"
...
LATEST_TAR=$(ls -t "$BACKUP_DIR"/ethosenv_wordpress_*.tar.gz 2>/dev/null | head -1 || echo "")
LATEST_INFO=$(ls -t "$BACKUP_DIR"/ethosenv_backup_info_*.json 2>/dev/null | head -1 || echo "")
