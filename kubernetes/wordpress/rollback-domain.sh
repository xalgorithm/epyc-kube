...
echo "Rolling back domain change from kampfzwerg.me to metaphysicalninja.com..."

# Update ingress back to the original domain
echo "Updating ingress configuration..."
# Update ingress.yaml first
sed -i '' 's/kampfzwerg.me/metaphysicalninja.com/g' ingress.yaml
...
sed -i '' 's/kampfzwerg.me/metaphysicalninja.com/g' wordpress-deployment.yaml
...
kubectl exec -n wordpress $WORDPRESS_POD -- wp search-replace 'https://kampfzwerg.me' 'https://metaphysicalninja.com' --all-tables || echo "Warning: Failed to update HTTPS URLs in database"

# Also update without https to catch any http URLs
kubectl exec -n wordpress $WORDPRESS_POD -- wp search-replace 'http://kampfzwerg.me' 'https://metaphysicalninja.com' --all-tables || echo "Warning: Failed to update HTTP URLs in database"
