...
HTTP_STATUS=$(curl -s -o /dev/null -w "%{http_code}" https://grafana.xalg.im)
...
AUTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" https://login.xalg.im/realms/xalg-apps/protocol/openid-connect/auth)
...
if curl -s https://grafana.xalg.im/login | grep -q "Keycloak-OAuth\|generic_oauth"; then
...
echo "  - Redirect URI: https://grafana.xalg.im/login/generic_oauth"
echo ""
...
echo "1. Visit https://grafana.xalg.im"
