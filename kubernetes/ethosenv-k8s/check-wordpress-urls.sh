...
echo ""
log_info "Searching for ethos.xalg.im URLs:"
kubectl exec -n "$NAMESPACE" "$MYSQL_POD" -- mysql -u "$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" -e "SELECT option_name, option_value FROM wp_options WHERE option_value LIKE '%ethos.xalg.im%';" 2>/dev/null
