...
kubectl patch deployment wordpress -n wordpress --type=json -p '[
  {
    ...
    "value": "define('\''WP_HOME'\'', '\''https://kampfzwerg.me'\'');\ndefine('\''WP_SITEURL'\'', '\''https://kampfzwerg.me'\'');\n"
  }
]'
...
kubectl exec -n wordpress $WORDPRESS_POD -- wp search-replace 'https://metaphysicalninja.com' 'https://kampfzwerg.me' --all-tables || echo "Warning: Failed to update HTTPS URLs in database"

# Also update without https to catch any http URLs
kubectl exec -n wordpress $WORDPRESS_POD -- wp search-replace 'http://metaphysicalninja.com' 'https://kampfzwerg.me' --all-tables || echo "Warning: Failed to update HTTP URLs in database"

echo "Domain update complete. The WordPress site should now be accessible at https://kampfzwerg.me"
