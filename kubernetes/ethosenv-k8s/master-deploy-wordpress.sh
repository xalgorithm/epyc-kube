...
CORRECT_URL="https://ethos.xalg.im"
...
if [ -z "$TABLES_CHECK" ]; then
    ...
    URL_PATTERNS=(
        ...
        "http://localhost"
        "https://localhost"
        "http://ethos.xalg.im:8080"
        "https://ethos.xalg.im:8080"
        "http://ethos.xalg.im"
    )
...
curl -I http://ethos.xalg.im 2>/dev/null && log_success "✅ HTTP responds" || log_warning "⚠️  HTTP test failed"

echo ""
echo "Testing HTTPS:"
curl -I https://ethos.xalg.im 2>/dev/null && log_success "✅ HTTPS responds" || log_warning "⚠️  HTTPS test failed"

log_substep "Checking DNS resolution..."
nslookup ethos.xalg.im 2>/dev/null && log_success "✅ DNS resolves" || log_warning "⚠️  DNS resolution failed"
