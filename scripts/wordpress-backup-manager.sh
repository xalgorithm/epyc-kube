...
# WordPress (Kampfzwerg) Backup Manager
# Manages backups, restoration, and cleanup for WordPress site
...
show_usage() {
    echo -e "${BLUE}WordPress (Kampfzwerg) Backup Manager${NC}"
    echo -e "${BLUE}====================================${NC}"
    echo ""
...
create_backup() {
    echo -e "${BLUE}Creating new WordPress backup...${NC}"
    ./scripts/backup-wordpress.sh
}

list_backups() {
    echo -e "${BLUE}Available WordPress Backups${NC}"
    echo -e "${BLUE}===========================${NC}"
    ...
    ls -lah "$BACKUP_DIR"/wordpress_database_*.sql 2>/dev/null | while read -r line; do
        echo "  $line"
    done || echo "  No SQL backups found"
    
    echo ""
    echo -e "${YELLOW}WordPress File Backups:${NC}"
    ls -lah "$BACKUP_DIR"/wordpress_files_*.tar.gz 2>/dev/null | while read -r line; do
        echo "  $line"
    done || echo "  No WordPress backups found"
    
    echo ""
    echo -e "${YELLOW}Backup Info Files:${NC}"
    ls -lah "$BACKUP_DIR"/wordpress_backup_info_*.json 2>/dev/null | while read -r line; do
        echo "  $line"
...
verify_backup() {
    ...
    ./scripts/verify-wordpress-backup.sh
}
...
cleanup_backups() {
    ...
    # Clean SQL backups
    SQL_COUNT=$(ls -1 "$BACKUP_DIR"/wordpress_database_*.sql 2>/dev/null | wc -l || echo 0)
    if [ "$SQL_COUNT" -gt 5 ]; then
        echo -e "${YELLOW}Removing old SQL backups...${NC}"
        ls -t "$BACKUP_DIR"/wordpress_database_*.sql | tail -n +6 | xargs rm -f
    ...
    WP_COUNT=$(ls -1 "$BACKUP_DIR"/wordpress_files_*.tar.gz 2>/dev/null | wc -l || echo 0)
    if [ "$WP_COUNT" -gt 5 ]; then
        echo -e "${YELLOW}Removing old WordPress backups...${NC}"
        ls -t "$BACKUP_DIR"/wordpress_files_*.tar.gz | tail -n +6 | xargs rm -f
    ...
    # Clean info files
    INFO_COUNT=$(ls -1 "$BACKUP_DIR"/wordpress_backup_info_*.json 2>/dev/null | wc -l || echo 0)
    if [ "$INFO_COUNT" -gt 5 ]; then
        echo -e "${YELLOW}Removing old info files...${NC}"
        ls -t "$BACKUP_DIR"/wordpress_backup_info_*.json | tail -n +6 | xargs rm -f
    ...
    MANIFEST_COUNT=$(ls -1 "$BACKUP_DIR"/wordpress_backup_manifest_*.txt 2>/dev/null | wc -l || echo 0)
    if [ "$MANIFEST_COUNT" -gt 5 ]; then
        echo -e "${YELLOW}Removing old manifest files...${NC}"
        ls -t "$BACKUP_DIR"/wordpress_backup_manifest_*.txt | tail -n +6 | xargs rm -f
        echo -e "${GREEN}✓ Removed $((MANIFEST_COUNT - 5)) old manifest files${NC}"
    else
        echo -e "${GREEN}✓ Manifest files: $MANIFEST_COUNT (no cleanup needed)${NC}"
    fi
...
show_info() {
    echo -e "${BLUE}WordPress (Kampfzwerg) Backup Information${NC}"
    echo -e "${BLUE}=========================================${NC}"
    ...
    if [ -d "$BACKUP_DIR" ]; then
        ...
        LATEST_WP=$(ls -t "$BACKUP_DIR"/wordpress_files_*.tar.gz 2>/dev/null | head -1 || echo "")
...
show_restore_instructions() {
    echo -e "${BLUE}WordPress (Kampfzwerg) Restoration Instructions${NC}"
    echo -e "${BLUE}===============================================${NC}"
    echo ""
    ...
    echo "   tar -xzf wordpress_files_TIMESTAMP.tar.gz"
    echo ""
